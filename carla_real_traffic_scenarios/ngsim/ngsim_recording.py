...
class NGSimRecording(Simulator):
    ...
    def step(self) -> List[NGSimCar]:
        ...
        for vehicle_id in vehicles_ids:
            ...
            info_df = df[now & this_vehicle]
            vehicle_info = VehicleInfo(info_df, self.offset, kernel=self.smoothing_window, mapper=self._mapper)
            self.VehicleInfoList.append(vehicle_info)
